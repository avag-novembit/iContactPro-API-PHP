...
class iContactProApi {
	...
	private	  $customFieldKeys	 = array(); // Holds iContact subscriptions custom fields
	...
	**/
	 ...
	 * @return void
	 */
	public function setCustomFieldKeys ( $customFieldKeys ) {
		if( ! empty( $customFieldKeys ) ) $this->customFieldKeys = $customFieldKeys;
	}
	 ...
	 * @return object
	**/
	public function addContact($sEmail, $sStatus = 'normal', $sPrefix = null, $sFirstName = null, $sLastName = null, $sSuffix = null, $sStreet = null, $sStreet2 = null, $sCity = null, $sState = null, $sPostalCode = null, $sPhone = null, $sFax = null, $sBusiness = null, $customFields = array()) {
		// Valid statuses
		...
		// In case custom fields are provided add to request
		if( ! empty( $this->customFieldKeys ) ) {
			foreach( $customFields as $key => $customField ) {
				if( ! in_array( $key, $this->customFieldKeys ) ) continue;
				$aContact[$key] = $customField;
			}
		}
	...
	**/
	 ...
	 * @param bool   $removeBrackets   whether remove json object brackets or not, since the API needs differently formatted json objects for different requests
	 *
	 * @return array|object
	 *
	 * @throws Exception if there are missing parameters or failed the API call
	**/
	public function makeCall($sResource, $sMethod = 'GET', $mPostData = null, $sReturnKey = null, $removeBrackets = false) {
		...
		switch (strtoupper($sMethod)) {
			...
			case 'POST'   :
				if (empty($mPostData)) {
					...
					$formattedPostData = $removeBrackets
						? str_replace( array( '[', ']' ), '', htmlspecialchars( json_encode( $mPostData ), ENT_NOQUOTES ) )
						: json_encode($mPostData);
					curl_setopt($rHandle, CURLOPT_POST, true);
					curl_setopt($rHandle, CURLOPT_POSTFIELDS, $formattedPostData);
					$this->sLastRequest = (string) $formattedPostData;
				}
			break;
			// Uploading data
			case 'PUT'    :
	...
	**/
	 ...
	 * @return bool|object
	 **/
	public function updateContact($iContactId, $sEmail = null, $sPrefix = null, $sFirstName = null, $sLastName = null, $sSuffix = null, $sStreet = null, $sStreet2 = null, $sCity = null, $sState = null, $sPostalCode = null, $sPhone = null, $sFax = null, $sBusiness = null, $sStatus = null, $customFields = array()) {
		// Valid statuses
		$aValidStatuses = array('normal', 'bounced', 'donotcontact', 'pending', 'invitable', 'deleted');
		// Contact placeholder
		$aContact       = array();
		...
		if (!empty($aContact)) {
			// Make the call
			$oContact = $this->makeCall("/a/{$this->getCompanyId()}/c/{$this->getProfileId()}/contacts/{$iContactId}", 'POST', array($aContact), 'contact', true);
			// Return the contact
			return $oContact;
		}
	...
	**/
	...
	public function deleteContact( $iContactId ) {
		return $this->makeCall("/a/{$this->getCompanyId()}/c/{$this->getProfileId()}/contacts/{$iContactId}", 'DELETE', null, 'contact');
	}
	
	/**
	 * Moves a subscription from one list to another
	 * 
	...
	public function moveSubscriptionToList($sSubscriptionId, $iListId, $sStatus = 'normal') {
		...
		// Setup the subscription and make the call
		$aSubscription = $this->makeCall("/a/{$this->getCompanyId()}/c/{$this->getProfileId()}/subscriptions/{$sSubscriptionId}", 'PUT', array(						
			'listId'    => $iListId,
			'status'    => $sStatus
		));
	...
	**/
	public function getHeaders() {
		...
		return array(
			...
			'Api-Version:  ' . (defined('ICP_APIVERSION')       	  ? constant('ICP_APIVERSION') 		: '2.3'),
